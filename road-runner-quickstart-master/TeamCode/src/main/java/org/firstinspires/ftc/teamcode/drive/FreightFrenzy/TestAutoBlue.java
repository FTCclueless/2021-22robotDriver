...
public class TestAutoBlue extends LinearOpMode {
    ...
    public void driveOut(Pose2d endPoint){
        ...
        /*
        while (drive.slidesCase == 4 && drive.slideTime <= 150 && opModeIsActive()){
            drive.update();
            Pose2d error = drive.getRelError(newEnd);
            if (Math.abs(error.getY()) <= 0.5){
                error = new Pose2d(error.getX(), 0, 0);
            }
            drive.updateMotors(error, 0.215, 0.45,14, Math.toRadians(8), 0.35, 0.5, 0.15 * side);
        }
         */
    ...
    public void intakeMineral(double power, long maxTime, boolean goBackIfStall){
        ...
        while(opModeIsActive() && drive.intakeCase <= 2 && a-startingTime <= maxTime && a - start <= 30000 - cutoff){
            ...
            if (a - start >= 550 && Math.abs(drive.relCurrentVelocity.getX()) <= 2 && goBackIfStall){
            ...
            if (Math.abs(drive.intakeSpeed) >= 3){
                stall = a;
            }
