...
public class Localizer implements com.acmerobotics.roadrunner.localization.Localizer {
    ...
    public Localizer(){
        ...
        encoders[2] = new Encoder(new Vector2d(6.531451276593018,3),  -1.0);
    }
    ...
    public void update() {
        ...
        currentVel = new Pose2d(
                (deltaLoops.getX()/totalTime)*gain + currentVel.getX()*(1.0-gain),
                (deltaLoops.getY()/totalTime)*gain + currentVel.getY()*(1.0-gain),
                (deltaLoops.getHeading()/totalTime)*gain + currentVel.getHeading()*(1.0-gain)
        );
        relCurrentVel = new Pose2d(
                (total.getX()/totalTime)*gain + relCurrentVel.getX()*(1.0-gain),
                (total.getY()/totalTime)*gain + relCurrentVel.getY()*(1.0-gain),
